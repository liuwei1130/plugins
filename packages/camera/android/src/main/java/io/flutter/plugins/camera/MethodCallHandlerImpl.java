...
final class MethodCallHandlerImpl implements MethodChannel.MethodCallHandler {
    ...
    private static boolean sIsCheckPermission;
    private @Nullable
    Camera camera;
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull final Result result) {
        sIsCheckPermission = false;
        switch (call.method) {
            case "requestForPermission":
                sIsCheckPermission = true;

                cameraPermissions.requestPermissions(activity, permissionsRegistry,
                        false,
                        new CameraPermissions.ResultCallback() {
                            @Override
                            public void onResult(String errorCode, String errorDescription) {
                                if (!sIsCheckPermission) {
                                    return;
                                }
                                if (errorCode == null) {
                                    result.success(true);
    ...
    private void turnOnOrOff(boolean isTurnOn, final Result result) {
        result.success(camera.turnOnOrOff(isTurnOn));
    }
